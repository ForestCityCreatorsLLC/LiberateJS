# Ultimate Multi-Agent Standalone Standalone Migration & GitHub Deploy Orchestration Prompt
...
```markdown
You are the Master Orchestrator Agent in the Anti-Gravity IDE. Your objective is to take the web application in my current active workspace (originally built on the proprietary "standalone" platform), strip out all standalone dependencies, rework it into a premium standalone application, and push the resulting codebase to a new repository on GitHub.
...
1. **Cleanser & Dependency Agent (Role: Codebase Cleanser)**:
   - *Task*: Handles file ingestion validation, verifies credentials for private sources, runs the automated cleanup script to purge standalone dependencies, and executes initial package installations.
...
1. **Source Validation**:
   ...
   - If files are NOT present and need to be pulled from a remote Standalone source:
...
2. **Run Cleanser**:
   - Execute the global Python converter script on the workspace directory (`.`):
     ```powershell
     python "C:\Users\igxxg\.gemini\antigravity\scratch\standalone-converter\decouple-cleanse.py" --dir . --recipe recipes/standalone.json
     ```
...
2. **Router & State Adapters**:
   - Locate proprietary wrappers (e.g. `<StandaloneWrapper>`, `<StandaloneWrapper>`).
   - Replace them with standard React router contexts (`react-router-dom`) or custom global contexts.
   - Map proprietary state management hooks (e.g. `useStandaloneState`) to standard React state or standard libraries (like Zustand).
3. **Environment Mapping**:
   - Replace references to old configuration files or parameters (like `standaloneParams` or `app-params.js`) with standard environment variable hooks (`import.meta.env` for Vite, or `process.env` for Webpack).
4. **Aesthetic and Layout Restoration**:
   - Check if removing the standalone template wrapper has left the UI looking basic or unstyled.
...
2. **Git User Check**:
   ...
   - If not configured, set them:
     ```powershell
     git config user.name "Standalone Migrator"
...
- **Safety**: Do not overwrite important business logic files; only target standalone wrapper infrastructure.
